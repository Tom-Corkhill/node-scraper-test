...
function internationalPhones(knwl) {
    ...
    this.calls = function() {
        ...
        let phoneRegEx = /^\d{3,11}$/;
        let countryCodeRegEx = /^\d{1,2}$/;
        ...
        for (let i = 0; i < arrWords.length; i++) {
            ...
            if (phoneRegEx.test(strCurrWord)) {
                ...
                if (i > 0 && strCurrWord.length === 10) {
                    ...
                    if (countryCodeRegEx.test(strCountryCode)) {
                        isCountryCode = true;
                    ...
                    if (strPrevWord.length === 5 && phoneRegEx.test(strPrevWord)) {
                        strCurrWord = strPrevWord + strCurrWord;
                    } else if ((strPrevWord.length === 4) && (phoneRegEx.test(strPrevWord)) && (countryCodeRegEx.test(strCountryCode))) {
                        isCountryCode = true;
                    ...
                    made up of 3 or 4 digits, possibly including a country code
                       If 3 digits long then it can also have a 3 digit previous word with a 5 digit area code before that */
                    let strPrevWord = knwl.tasks.removeCharacters(arrRemoveCharacters, arrWords[i-1]);
                    ...
                    if ((phoneRegEx.test(strPrevWord) && phoneRegEx.test(strSecondPrevWord))) {
                        if(strPrevWord.length === 3 && strSecondPrevWord.length === 5) {
                        ...
                        if (((strPrevWord.length === 3) || (strPrevWord.length === 4)) && ((strSecondPrevWord.length === 3) || (strSecondPrevWord.length === 4))) {
                            if (strCurrWord.length + strPrevWord.length + strSecondPrevWord.length === 11) {
                                strCurrWord = strSecondPrevWord + strPrevWord + strCurrWord;
                            } else if ((strCurrWord.length + strPrevWord.length + strSecondPrevWord.length === 10) && (countryCodeRegEx.test(strCountryCode))) {
                                isCountryCode = true;
